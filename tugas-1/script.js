...
const proflinkInput = document.getElementById("proflink");
const levelSelect = document.getElementById("level");
const waCheckbox = document.getElementById("join-dc");
...
function renderParticipants() {
    ...
    for (const p of participants) {
        ...
        li.textContent = `${p.name} – ${p.level} ${p.joinWA ? "(Join Discord Server)" : ""}`;
...
form.addEventListener("submit", (event) => {
    ...
    const profile = proflinkInput.value.trim();
    ...
    if (!profile.includes("steamcommunity")) {
        errorEl.textContent = "Please enter a valid Steam profile link.";
        successEl.textContent = "";
        return;
    }
    ...
    addParticipant({ name, email: profile, level, joinWA });
